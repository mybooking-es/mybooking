	...
	function mybooking_site_logo() {
      ...
      if ( isset($post) ) {
        ...
        if ( !empty( $mybooking_custom_logo ) ) { 
          $result = wp_kses_post( sprintf('<a href="%s" class="navbar-brand custom-logo-link" rel="home"><img src="%s" class="img-fluid"></a>',
							            								esc_url( home_url( '/' ) ),
...
EOT;
		...
		return wp_kses_post( $result ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	}
}

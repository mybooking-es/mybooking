...
class Customizer_Alpha_Color_Control extends WP_Customize_Control {
	...
	public function enqueue() {
		wp_enqueue_script(
			...
			get_template_directory_uri() . '/js/alpha-color-picker.js',
		...
		wp_enqueue_style(
			'customizer-alpha-color-picker',
			get_template_directory_uri() . '/css/alpha-color-picker.css',
	...
	public function render_content() {
		...
		?>
		<label>
			<input class="alpha-color-control" type="text" size="10" 
						 data-show-opacity="<?php echo esc_attr( $show_opacity ); ?>" 
						 data-palette="<?php echo esc_attr( $palette ); ?>" 
						 data-default-color="<?php echo esc_attr( $this->settings['default']->default ); ?>" <?php esc_attr( $this->link() ); ?>  />
		</label>
		<?php
	}
}
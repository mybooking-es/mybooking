...
<script type="text/tmpl" id="form_calendar_selector_tmpl">
  <% if (configuration.pickupReturnPlace) { %> 
        ...
        <div class="form-group"  class="pickup_place_group">
          <label
            for="pickup_place"><?php echo _x( 'Select pick-up place', 'renting_product_calendar', 'mybooking') ?></label>
        ...
        <div class="form-group" class="return_place">
          <label
            for="pickup_place"><?php echo _x( 'Select return place', 'renting_product_calendar', 'mybooking' ) ?></label>
...
<script type="text/tmpl" id="script_reservation_summary">
  ...
  <% if (product_available) { %>    
    <h4 class="brand-primary my-3">
      <?php echo _x( 'Reservation summary', 'renting_product_calendar', 'mybooking') ?></h4>

    <% if (shopping_cart.days > 0) { %>
    <p class="color-gray-600"><?php echo _x( 'Rental duration', 'renting_product_calendar', 'mybooking' ) ?>:
      <span><%=shopping_cart.days%>
        <?php echo _x( 'day(s)', 'renting_product_calendar', 'mybooking' ) ?></span></p>
    <% } else if (shopping_cart.hours > 0) { %>
    <p class="color-gray-600"><?php echo _x( 'Rental duration', 'renting_product_calendar', 'mybooking' ) ?>:
      <span><%=shopping_cart.hours%>
        <?php echo _x( 'hours(s)', 'renting_product_calendar', 'mybooking' ) ?></span></p>
    <% } %>

    <h5><?php echo _x('Product', 'renting_product_calendar', 'mybooking') ?></h5>
    <p class="color-gray-600"><%=configuration.formatCurrency(shopping_cart.item_cost)%></p>

    <% if (shopping_cart.time_from_cost > 0) { %>
    <h6><?php echo _x('Pick-up time supplement', 'renting_product_calendar', 'mybooking') ?></h6>
    <p class="color-gray-600"><%=configuration.formatCurrency(shopping_cart.time_from_cost)%></p>
    <% } %>

    <% if (shopping_cart.pickup_place_cost > 0) { %>
    <h6><?php echo _x('Pick-up place supplement', 'renting_product_calendar', 'mybooking') ?></h6>
    <p class="color-gray-600"><%=configuration.formatCurrency(shopping_cart.pickup_place_cost)%></p>
    <% } %>

    <% if (shopping_cart.time_to_cost > 0) { %>
    <h6><?php echo _x('Return time supplement', 'renting_product_calendar', 'mybooking') ?></h6>
    <p class="color-gray-600"><%=configuration.formatCurrency(shopping_cart.time_to_cost)%></p>
    <% } %>

    <% if (shopping_cart.return_place_cost > 0) { %>
    <h6><?php echo _x('Return place supplement', 'renting_product_calendar', 'mybooking') ?></h6>
    <p class="color-gray-600"><%=configuration.formatCurrency(shopping_cart.return_place_cost)%></p>
    <% } %>

    <h4 class="color-brand-primary my-3">
    <?php echo _x( 'Total', 'renting_product_calendar', 'mybooking' ) ?>
    </h4>
  ...
  <% } else { %>
    <% if (product_type == 'resource') { %>
      <div class="alert alert-danger">
        <p>
          <?php echo _x( 'Sorry, there is no availability during these hours', 'renting_product_calendar', 'mybooking') ?>
        </p>
      </div>
    <% } else if (product_type == 'category_of_resources') { %>
      <div class="alert alert-warning">
        <p>
          <?php echo _x( 'Sorry, there is no availability for the entire period. The calendar shows those days when there is availability, but it may not be available for certain consecutive dates.', 'renting_product_calendar', 'mybooking') ?>
        </p>
        <p></p>
      </div>
    <% } %>
